...
function validateZipCode(zip, state) {
  ...
  const stateZipRanges = {
    ...
    'TX': ['750', '751', '752', '753', '754', '755', '756', '757', '758', '759', '760', '761', '762', '763', '764', '765', '766', '767', '768', '769', '770', '771', '772', '773', '774', '775', '776', '777', '778', '779', '780', '781', '782', '783', '784', '785', '786', '787', '788', '789', '790', '791', '792', '793', '794', '795', '796', '797', '798', '799', '885'],
    'FL': ['320', '321', '322', '323', '324', '325', '326', '327', '328', '329', '330', '331', '332', '333', '334', '335', '336', '337', '338', '339', '340', '341', '342', '343', '344', '345', '346', '347']
  };
...
function validateBudgetPropertyAlignment(budget, propertyType) {
  ...
  const invalidCombos = [
    { budget: '$15,000+', property: 'Apartment (rent)', reason: 'Budget too high for rental apartment' },
    { budget: 'Under $500', property: 'Commercial property', reason: 'Budget too low for commercial property' },
    { budget: '$10,000-$14,999', property: 'Apartment (rent)', reason: 'Budget unusual for rental apartment' }
  ];
...
async function calculateLeadScore(leadData, prisma) {
  ...
  if (!phoneCheck.valid) {
    ...
    const caAreaCodes = ['213', '310', '323', '424', '442', '510', '530', '559', '562', '619', '626', '628', '650', '657', '661', '669', '707', '714', '747', '760', '805', '818', '820', '831', '858', '909', '916', '925', '949', '951'];
    if (caAreaCodes.includes(phoneCheck.areaCode) && zipPrefix?.startsWith('9')) {
  ...
  const alignmentCheck = validateBudgetPropertyAlignment(leadData.budget_range, leadData.property_type);
  ...
  if (prisma) {
    ...
    if (duplicateEmail) {
      validationErrors.push('You already submitted a request with this email in the last 7 days');
    }
    
    if (duplicatePhone && !duplicateEmail) {
      validationErrors.push('You already submitted a request with this phone number in the last 7 days');
    }
  }
  ...
  if (leadData.service_description) {
    ...
    const urgencyKeywords = ['emergency', 'urgent', 'asap', 'immediately', 'today', 'now', 'help'];
